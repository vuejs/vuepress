...
module.exports = class PluginAPI {
  constructor (context) {
    ...
    this._pluginResolver = moduleResolver.getPluginResolver()
  ...
  use (pluginRaw, pluginOptions = {}) {
    ...
    let plugin
    if (datatypes.isPlainObject(pluginRaw) && pluginRaw.$$normalized) {
      plugin = pluginRaw
    } else {
      try {
  ...
  registerOption (key, value, pluginName) {
    ...
    const { valid, warnMsg } = datatypes.assertTypes(value, types)
    if (valid) {
