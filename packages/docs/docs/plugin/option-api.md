...
If you want to pass an object to the client, you can also use the `data` attribute:
...
::: warning NOTE
The `data` property will be converted to a string, so the function does not have any context. If you use variables in a closure, you need to provide it yourself with the `content` attribute. The code generated by `data` and `content` will be placed in the same file.
:::
