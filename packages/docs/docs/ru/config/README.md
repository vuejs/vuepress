...
Укажите каталог сборки для `vuepress build`. Если указан относительный путь, он будет разрешен на основе `process.cwd()`.
...
VuePress по умолчанию использует [cache-loader](https://github.com/webpack-contrib/cache-loader), чтобы значительно ускорить компиляцию webpack.

Эта опция используется для указания пути к кэшу, а также может удалять кэш перед каждой сборкой, для этого установите значение `false`

::: tip
Эту опцию также можно указать через команду CLI:
...
Укажите дополнительные файлы для отслеживания.

Вы можете отслеживать любые файлы, если хотите. Изменения в файлах приведут к пересборке `vuepress` и обновлению в реальном времени.
...
Если вы просто хотите изменить цвета [темы по умолчанию](https://github.com/vuejs/vuepress/blob/master/packages/@vuepress/core/lib/app/style/config.styl ) или определить дополнительные цветовые переменные для последующего использования, вы можете создать файл `.vuepress/styles/palette.styl`.
...
Вы должны использовать этот файл ТОЛЬКО для указания переменных цвета, `palette.styl` добавляется в конец основного файла настроек stylus и будет использоваться несколькими файлами, если вы напишете здесь стили, они будут дублироваться несколько раз.
:::
...
Укажите это, чтобы использовать собственную тему. При значении `"foo"` VuePress попытается загрузить компонент темы из `node_modules/vuepress-theme-foo/Layout.vue`.
...
Функция преобразования текста заголовков в семантический URL. Влияет на идентификаторы/ссылки, сгенерированные для якорей заголовков, оглавления и ссылки на боковой панели.
...
Функция для изменения конфигурации по умолчанию или добавления других плагинов к [markdown-it](https://github.com/markdown-it/markdown-it), используемому для преобразования исходных файлов. Например
...
::: tip
Эта опция также используется в [Plugin API](../plugin/option-api.md#extendmarkdown).
:::
...
Параметры для [postcss-loader](https://github.com/postcss/postcss-loader). Обратите внимание, что при указании этого значения `autoprefixer` будет перезаписан, вам нужно будет добавить его самостоятельно.
...
Опции для [sass-loader](https://github.com/webpack-contrib/sass-loader) для обработки `*.scss` файлов.
...
Опции для [sass-loader](https://github.com/webpack-contrib/sass-loader) для обработки `*.sass` файлов.
...
Опции для [less-loader](https://github.com/webpack-contrib/less-loader) для обработки `*.less`.
