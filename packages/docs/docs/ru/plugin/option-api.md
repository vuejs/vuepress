...
VuePress будет использовать имя пакета плагина в качестве имени плагина. Если ваш плагин является локальным (то есть напрямую использует функцию чистого плагина), обязательно настройте эту опцию, которая подходит для отладки.
...
::: tip
Поскольку VuePress является приложением на основе Vue-SSR, будет две конфигурации webpack, `isServer` используется для определения того, применяется ли текущая конфигурация webpack к серверу или клиенту.
...
Опция принимает абсолютные пути к файлам, указывающие на файлы расширений, или функцию, которая возвращает пути, которая позволит вам выполнить некоторые [Улучшения на уровне приложения](../guide/basic-config.md#улучшения-на-уровне-приложения)
...
Опция также поддерживает динамический код, который позволит вам делать больше вещей с возможностью использовать контекст сборки:
...
Вы можете захотеть сгенерировать некоторые клиентские модули во время сборки.
...
Функция, используемая для расширения или изменения объекта [$page](../guide/global-computed.md#page). Эта функция будет вызываться один раз для каждой страницы во время сборки.
...
Затем вы можете использовать это значение с помощью `this.$page.size` в любом компоненте Vue.
...
Затем VuePress автоматически внедрит эти компоненты после компонента макета:
...
Зарегистрируйте дополнительную команду CLI vuepress. Функция будет вызываться с экземпляром [CAC](https://github.com/cacjs/cac) в качестве первого аргумента.
...
::: tip
Обратите внимание, что пользовательская команда, зарегистрированная плагином, требует, чтобы VuePress определял конфигурацию вашего сайта, например, `vuepress dev` и `vuepress build`, поэтому при разработке команды обязательно сообщите пользователю указать `targetDir` в качестве аргумента CLI.
:::
