...
```md
[Главная](/) <!-- Отправляет пользователя в корень README.md -->
[пример](/foo/) <!-- Отправляет пользователя в index.html каталога foo -->
[пример якорь заголовка](/foo/#heading) <!-- Привязывает пользователя к заголовку в файле foo README -->
...
``` yaml
---
title: Блоггинг словно хакер
lang: ru-RU
---
```

Данные будут доступны для остальной части страницы, а также для всех пользовательских и тематических компонентов как `$page`.

`title` и `lang` будет автоматически установлены на текущей странице. Кроме того, вы можете указать дополнительные метатеги для внедрения:
