...
Скомпилированный компонент не имеет личных данных, но имеет доступ к [метаданным сайта](./custom-themes.md#метаданные-сайта-и-страницы). Например:
...
Вывод HTML выполняется с помощью [markdown-it](https://github.com/markdown-it/markdown-it), тогда как анализируемые заголовки выполняются VuePress и используются для [sidebar](../default-theme-config/README.md#sidebar) и заголовок документа.
:::
...
Скомпилированное содержимое текущего файла `.md` Это будет очень полезно при использовании [Пользовательского Макета](../default-theme-config/README.md#пользовательский-макет-для-определенных-страниц).
