...
Стандартная тема предоставляет шаблон главной страницы (который используется [на главной странице этого сайта](../README.md)). Чтобы использовать её, укажите `home: true` и необходимую информацию в корневом `README.md` [YAML front matter](../guide/markdown.md#yaml-front-matter). Она и будет использоваться на сайте:
