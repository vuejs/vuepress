...
module.exports = (md, {
  ...
  md.renderer.rules.link_open = (tokens, idx, options, env, self) => {
    ...
    if (hrefIndex >= 0) {
      ...
      if (isExternal) {
        // 1. Add attributes to a external link.
        Object.entries(externalAttrs).forEach(([key, val]) => {
          token.attrSet(key, val)
        })
        // 2. Only add symbol to a external link that opens a blank page.
        if (externalLinkSymbol && /_blank/i.test(externalAttrs['target'])) {
  ...
  md.renderer.rules.link_close = (tokens, idx, options, env, self) => {
    ...
    if (externalLinkSymbol && hasOpenExternalLink) {
