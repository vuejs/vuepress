...
module.exports = async function resolveOptions (sourceDir) {
  ...
  const pluginContextOptions = { sourceDir, outDir }
  const pluginContext = new PluginContext(pluginContextOptions)
  const plugin = new Plugin(plugins, pluginContext)
  ...
  const pagesData = await Promise.all(pageFiles.map(async (relative) => {
    ...
    await plugin.apis.extendPageData.run({ data, filepath, relative, key, sourceDir, content })
  ...
  Object.assign(pluginContextOptions, options)
