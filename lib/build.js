module.exports = async function build (sourceDir, cliOptions = {}) {
  ...
  const { normalizeHeadTag, applyUserWebpackConfig, fileNameConverter } = require('./util')
  ...
  if (options.siteConfig.sitemap) {
    const siteMap = sitemap(options)
    const smPath = path.resolve(outDir, siteMap.filename)
    await fs.writeFile(smPath, siteMap.content, 'utf-8')
  }
  ...
  async function renderPage (page) {
    ...
    const filename = fileNameConverter(pagePath)
