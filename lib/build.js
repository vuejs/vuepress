module.exports = async function build (sourceDir, cliOptions = {}) {
  ...
  // write sitemap file
  if (options.siteConfig.sitemap) {
    const siteMapString = options.siteData.pages.reduce((prev, current) => {
      const filename = fileNameConverter(current.path)
      return prev.trim() + '\n' + options.siteConfig.sitemap.domain + options.siteData.base + filename
    }, '')
    const outFileName = options.siteConfig.sitemap.filename || 'sitemap.txt'
    const smPath = path.resolve(outDir, outFileName)
    await fs.writeFile(smPath, siteMapString, 'utf-8')
  }
  ...
  async function renderPage (page) {
    ...
    const filename = fileNameConverter(pagePath) // pagePath.replace(/\/$/, '/index.html').replace(/^\//, '')
